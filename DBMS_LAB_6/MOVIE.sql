...
-- 5. List all actors who acted in a movie before 2003 and also in a movie after 2004 (use JOIN operation).
SELECT A.ACT_NAME, MC.MOVIE_ID,M.MOVIE_TITLE, M.MOVIE_YEAR  FROM ACTOR A 
JOIN MOVIE_CAST MC ON A.ACT_ID=MC.ACT_ID
JOIN MOVIES M ON MC.MOVIE_ID=M.MOVIE_ID 
AND A.ACT_NAME IN (SELECT A.ACT_NAME FROM ACTOR A,MOVIE_CAST MC, MOVIES M WHERE A.ACT_ID=MC.ACT_ID AND MC.MOVIE_ID=M.MOVIE_ID GROUP BY A.ACT_NAME );
